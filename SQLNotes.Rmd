...
## Self JOINS
Sometimes it can be useful to join a table onto itself. Most of the time you'll do this in ordr to find cases where two events both occurred, one after the other. 
e.g. Imagine you want to know which accounts making multiple orders within 30 days. One way to do this would be to join the orders table onto itself with an inequality join. 
...
One thing to keep in mind when doing self joins is you'll need to use aliases to distinguish between the left and right tbales since it would be unclear if you were just to refer to one of them as orders in this case. 
```{sql}
...
Once you uNION two SELECT staments together you can perform iperations on the entire combined dataset rather than just on the individual parts. You can do this by UNIONing them together in a subquery so that the combined results are treated as a single results set. A cleaner way to do this might be to do the UNION in a common table expression and then do aggregations in the main query. 
...
e.g. If you've got iem series data limiting to a small time window can make your queries run much more quickly. 

Keep in ind you can always *perform exploratory analysis on a subset of data*, refine your work into a final query, then remove the limitation and run your work accross the entore dataset. The final query might take a long time to run but at least you can run the intermediate steps quickly. You can get a subset of data using *LIMIT* at the end of your query. It's worth noting that LIMIT doesn't quite work the same way with aggregations. The aggregation is oerformed first, then the results are limited to the specified number of rows. So If you're using aggregates LIMIT will do nothing to speed up your query. If you want to LIMIT the dataset before doing the aggregation to speed things up a bit more you'll need to do it in a subquery and keep the LIMIT within the subquery.
